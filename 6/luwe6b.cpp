        ...
        case 6:
            {
                ...
                PersonLista listayah = transaktioner.FixaPersoner();
                listayah.skrivUtOchFixa(cout);
                break;
            }
        }
    }
...
void PersonLista::laggTill(Person ny_person){
    Person *tmp = new Person[antal_personer + 1];
...
void TransaktionsLista::laggTill(Transaktion &t){
    Transaktion *tmp = new Transaktion[antal_transaktioner + 1];
    for(int i = 0; i < antal_transaktioner; i++){
        tmp[i] = transaktioner[i];
    }
    delete [] transaktioner;
